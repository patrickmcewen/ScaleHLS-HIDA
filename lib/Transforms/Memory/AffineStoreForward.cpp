...
static affine::AffineReadOpInterface
forwardStoreToLoad(affine::AffineReadOpInterface loadOp,
  ...
  affine::AffineWriteOpInterface lastWriteStoreOp = nullptr;

  for (auto *user : loadOp.getMemRef().getUsers()) {
    auto storeOp = dyn_cast<affine::AffineWriteOpInterface>(user);
    if (!storeOp)
      continue;
    ...
    if (startOp->getParentRegion() != loadOp->getParentRegion()) {
      if (auto ifOp = dyn_cast<affine::AffineIfOp>(storeOp->getParentOp()))
  ...
  if (!domInfo.dominates(lastWriteStoreOp, loadOp)) {
    ...
    auto ifOp = lastWriteStoreOp->getParentOfType<affine::AffineIfOp>();
    ...
    auto newLoad = cast<affine::AffineReadOpInterface>(builder.clone(*loadOp));
  ...
  // Record this to erase later.
  loadOpsToErase.push_back(loadOp);
  return affine::AffineReadOpInterface();
}
...
static void findUnusedStore(affine::AffineWriteOpInterface writeA,
  ...
  for (Operation *user : writeA.getMemRef().getUsers()) {
    ...
    if (!isa<affine::AffineWriteOpInterface>(user))
      continue;
    auto writeB = cast<affine::AffineWriteOpInterface>(user);
    ...
    if (targetA->getParentRegion() != targetB->getParentRegion()) {
      if (auto ifOpA = dyn_cast<affine::AffineIfOp>(writeA->getParentOp())) {
        if (isValid(ifOpA, writeB))
          targetA = ifOpA;
        if (auto ifOpB = dyn_cast<affine::AffineIfOp>(writeB->getParentOp()))
          if (checkSameIfStatement(ifOpA, ifOpB))
            targetB = ifOpB;
      } else if (auto ifOpB =
                     dyn_cast<affine::AffineIfOp>(writeB->getParentOp())) {
...
static void loadCSE(affine::AffineReadOpInterface loadA,
                    SmallVectorImpl<Operation *> &loadOpsToErase,
                    DominanceInfo &domInfo) {
  ...
  SmallVector<affine::AffineReadOpInterface, 4> loadCandidates;
  for (auto *user : loadA.getMemRef().getUsers()) {
    auto loadB = dyn_cast<affine::AffineReadOpInterface>(user);
  ...
  Value loadB;
  for (affine::AffineReadOpInterface option : loadCandidates) {
    if (llvm::all_of(loadCandidates, [&](affine::AffineReadOpInterface depStore) {
...
static bool applyAffineStoreForward(func::FuncOp func) {
  ...
  func.walk([&](affine::AffineReadOpInterface loadOp) {
    auto currentLoadOp = loadOp;
    auto newLoadOp = affine::AffineReadOpInterface();
    while (1) {
  ...
  func.walk([&](affine::AffineWriteOpInterface storeOp) {
