func.func @test_gemm(%arg0: f32, %arg1: f32, %arg2: memref<2x2xf32, #hls.partition<[none, cyclic], [1, 2]>, #hls.mem<lutram_2p>>, %arg3: memref<2x2xf32>, %arg4: memref<2x2xf32, #hls.partition<[none, cyclic], [1, 2]>, #hls.mem<lutram_2p>>) attributes {llvm.linkage = #llvm.linkage<external>, resource = #hls.res<lut = 0, dsp = 2, bram = 0>, timing = #hls.time<0 -> 44, latency = 44, interval = 44>, top_func} {
  affine.for %arg5 = 0 to 2 {
    ...
    } {loop_directive = #hls.loop<pipeline = true, target_ii = 8, dataflow = false, flatten = false>, loop_info = #hls.info<flatten_trip_count = 2, iter_latency = 16, min_ii = 8>, parallel, timing = #hls.time<0 -> 26, latency = 26, interval = 26>}
  } {loop_directive = #hls.loop<pipeline = false, target_ii = 1, dataflow = false, flatten = true>, loop_info = #hls.info<flatten_trip_count = 4, iter_latency = 16, min_ii = 8>, timing = #hls.time<0 -> 42, latency = 42, interval = 42>}
  return {timing = #hls.time<42 -> 42, latency = 0, interval = 0>}
}
